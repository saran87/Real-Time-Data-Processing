...
class BaseModel(object):
    ...
    def get_events(self, truck_id, package_id, is_above_threshold=True):
        ...
        query = {"truck_id": truck_id, "package_id": package_id, "is_processed": {"$exists":True}, "x": {"$exists": True}, "y": {"$exists":True}, "z": {"$exists": True}}
        ...
        if not cursor:
            ...
            query = {"truck_id":truck_id,"package_id":package_id, "is_processed": {"$exists":True}, "value.x":{"$exists":True}, "value.y": {"$exists":True}, "value.z": {"$exists":True}}
            cursor = self.get_cursor(query)
